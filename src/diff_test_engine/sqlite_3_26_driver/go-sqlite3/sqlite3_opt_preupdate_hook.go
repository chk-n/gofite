...
void preUpdateHookTrampoline_new(void*, sqlite3 *, int, char *, char *, sqlite3_3_26_int64, sqlite3_3_26_int64);
*/
import "C"
import (
	"errors"
...
func (c *SQLiteConn) RegisterPreUpdateHook(callback func(SQLitePreUpdateData)) {
	if callback == nil {
		C.sqlite3_3_26_preupdate_hook(c.db, nil, nil)
	} else {
		C.sqlite3_3_26_preupdate_hook(c.db, (*[0]byte)(unsafe.Pointer(C.preUpdateHookTrampoline_new)), unsafe.Pointer(newHandle(c, callback)))
	}
}

// Depth returns the source path of the write, see sqlite3_preupdate_depth()
func (d *SQLitePreUpdateData) Depth() int {
	return int(C.sqlite3_3_26_preupdate_depth(d.Conn.db))
}

// Count returns the number of columns in the row
func (d *SQLitePreUpdateData) Count() int {
	return int(C.sqlite3_3_26_preupdate_count(d.Conn.db))
}

func (d *SQLitePreUpdateData) row(dest []any, new bool) error {
	for i := 0; i < d.Count() && i < len(dest); i++ {
		var val *C.sqlite3_3_26_value
		var src any

		// Initially I tried making this just a function pointer argument, but
		// it's absurdly complicated to pass C function pointers.
		if new {
			C.sqlite3_3_26_preupdate_new(d.Conn.db, C.int(i), &val)
		} else {
			C.sqlite3_3_26_preupdate_old(d.Conn.db, C.int(i), &val)
		}

		switch C.sqlite3_3_26_value_type(val) {
		case C.SQLITE_INTEGER:
			src = int64(C.sqlite3_3_26_value_int64(val))
		case C.SQLITE_FLOAT:
			src = float64(C.sqlite3_3_26_value_double(val))
		case C.SQLITE_BLOB:
			len := C.sqlite3_3_26_value_bytes(val)
			blobptr := C.sqlite3_3_26_value_blob(val)
			src = C.GoBytes(blobptr, len)
		case C.SQLITE_TEXT:
			len := C.sqlite3_3_26_value_bytes(val)
			cstrptr := unsafe.Pointer(C.sqlite3_3_26_value_text(val))
