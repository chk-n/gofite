...
func doTestOpen(t *testing.T, option string) (string, error) {
	...
	db, err := sql.Open("sqlite3_new", url)
	if err != nil {
...
func TestOpenWithVFS(t *testing.T) {
	...
	db, err := sql.Open("sqlite3_new", fmt.Sprintf("file:%s?vfs=hello", filename))
	if err != nil {
	...
	db, err = sql.Open("sqlite3_new", fmt.Sprintf("file:%s?vfs=%s", filename, vfs))
	if err != nil {
		t.Fatal("Failed to open", err)
	}
	err = db.Ping()
...
func TestOpenNoCreate(t *testing.T) {
	...
	db, err := sql.Open("sqlite3_new", fmt.Sprintf("file:%s?mode=rw", filename))
	if err == nil {
		defer db.Close()
	...
	db, err = sql.Open("sqlite3_new", fmt.Sprintf("file:%s?mode=rwc", filename))
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
...
func TestReadonly(t *testing.T) {
	...
	db1, err := sql.Open("sqlite3_new", "file:"+tempFilename)
	if err != nil {
		t.Fatal(err)
	}
	db1.Exec("CREATE TABLE test (x int, y float)")

	db2, err := sql.Open("sqlite3_new", "file:"+tempFilename+"?mode=ro")
	if err != nil {
		t.Fatal(err)
	}
	_ = db2
...
func TestForeignKeys(t *testing.T) {
	...
	for option, want := range cases {
		...
		db, err := sql.Open("sqlite3_new", uri)
		if err != nil {
			os.Remove(fname)
...
func TestClose(t *testing.T) {
	tempFilename := TempFilename(t)
	defer os.Remove(tempFilename)
	db, err := sql.Open("sqlite3_new", tempFilename)
	if err != nil {
...
func TestTimezoneConversion(t *testing.T) {
	...
	for _, tz := range zones {
		tempFilename := TempFilename(t)
		defer os.Remove(tempFilename)
		db, err := sql.Open("sqlite3_new", tempFilename+"?_loc="+url.QueryEscape(tz))
		if err != nil {
			t.Fatal("Failed to open database:", err)
		}
		defer db.Close()
...
func TestDateTimeLocal(t *testing.T) {
	...
	tempFilename := TempFilename(t)
	defer os.Remove(tempFilename)
	db, err := sql.Open("sqlite3_new", tempFilename+"?_loc="+zone)
	if err != nil {
		t.Fatal("Failed to open database:", err)
	}
...
func TestFunctionRegistration(t *testing.T) {
	...
	sql.Register("sqlite3_3_26_FunctionRegistration", &SQLiteDriver{
	...
	db, err := sql.Open("sqlite3_3_26_FunctionRegistration", ":memory:")
	if err != nil {
		t.Fatal("Failed to open database:", err)
	}
	defer db.Close()
...
func TestAggregatorRegistration(t *testing.T) {
	...
	sql.Register("sqlite3_3_26_AggregatorRegistration", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
			return conn.RegisterAggregator("customSum", customSum, true)
		},
	})
	db, err := sql.Open("sqlite3_3_26_AggregatorRegistration", ":memory:")
	if err != nil {
		t.Fatal("Failed to open database:", err)
	}
	defer db.Close()
...
func TestAggregatorRegistration_GenericReturn(t *testing.T) {
	sql.Register("sqlite3_3_26_AggregatorRegistration_GenericReturn", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
			return conn.RegisterAggregator("mode", newMode, true)
		},
	})
	db, err := sql.Open("sqlite3_3_26_AggregatorRegistration_GenericReturn", ":memory:")
	if err != nil {
		t.Fatal("Failed to open database:", err)
	}
	defer db.Close()
...
func TestCollationRegistration(t *testing.T) {
	...
	sql.Register("sqlite3_3_26_CollationRegistration", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
	...
	db, err := sql.Open("sqlite3_3_26_CollationRegistration", ":memory:")
	if err != nil {
		t.Fatal("Failed to open database:", err)
	}
	defer db.Close()
...
func TestPinger(t *testing.T) {
	db, err := sql.Open("sqlite3_new", ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	err = db.Ping()
...
func TestUpdateAndTransactionHooks(t *testing.T) {
	...
	sql.Register("sqlite3_3_26_UpdateHook", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
	...
	db, err := sql.Open("sqlite3_3_26_UpdateHook", ":memory:")
	if err != nil {
		t.Fatal("Failed to open database:", err)
	}
	defer db.Close()
...
func TestAuthorizer(t *testing.T) {
	...
	sql.Register("sqlite3_3_26_Authorizer", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
	...
	db, err := sql.Open("sqlite3_3_26_Authorizer", ":memory:")
	if err != nil {
		t.Fatal("Failed to open database:", err)
	}
	defer db.Close()
...
func TestSetFileControlInt(t *testing.T) {
	t.Run("PERSIST_WAL", func(t *testing.T) {
		...
		sql.Register("sqlite3_3_26_FCNTL_PERSIST_WAL", &SQLiteDriver{
			ConnectHook: func(conn *SQLiteConn) error {
		...
		db, err := sql.Open("sqlite3_3_26_FCNTL_PERSIST_WAL", tempFilename)
		if err != nil {
			t.Fatal("Failed to open database:", err)
		}
		defer db.Close()
...
func TestSetFileControlInt64(t *testing.T) {
	...
	t.Run("", func(t *testing.T) {
		...
		sql.Register("sqlite3_3_26_FCNTL_SIZE_LIMIT", &SQLiteDriver{
			ConnectHook: func(conn *SQLiteConn) error {
		...
		db, err := sql.Open("sqlite3_new", "file:/dbname?vfs=memdb")
		if err != nil {
			t.Fatal("Failed to open database:", err)
		}
		err = db.Ping()
...
func BenchmarkCustomFunctions(b *testing.B) {
	customFunctionOnce.Do(func() {
		...
		sql.Register("sqlite3_3_26_BenchmarkCustomFunctions", &SQLiteDriver{
			ConnectHook: func(conn *SQLiteConn) error {
	...
	db, err := sql.Open("sqlite3_3_26_BenchmarkCustomFunctions", ":memory:")
	if err != nil {
...
func initializeTestDB(t testing.TB) {
	tempFilename := TempFilename(t)
	d, err := sql.Open("sqlite3_new", tempFilename+"?_busy_timeout=99999")
	if err != nil {
