...
func (c *SQLiteConn) Serialize(schema string) ([]byte, error) {
	...
	var sz C.sqlite3_3_26_int64
	ptr := C.sqlite3_3_26_serialize(c.db, zSchema, &sz, 0)
	if ptr == nil {
		return nil, fmt.Errorf("serialize failed")
	}
	defer C.sqlite3_3_26_free(unsafe.Pointer(ptr))

	if sz > C.sqlite3_3_26_int64(math.MaxInt) {
...
func (c *SQLiteConn) Deserialize(b []byte, schema string) error {
	...
	tmpBuf := (*C.uchar)(C.sqlite3_3_26_malloc64(C.sqlite3_3_26_uint64(len(b))))
	cBuf := *(*[]byte)(unsafe.Pointer(&reflect.SliceHeader{
	...
	rc := C.sqlite3_3_26_deserialize(c.db, zSchema, tmpBuf, C.sqlite3_3_26_int64(len(b)),
		C.sqlite3_3_26_int64(len(b)), C.SQLITE_DESERIALIZE_FREEONCLOSE)
	if rc != C.SQLITE_OK {
