...
func init() {
	...
	sql.Register("sqlite3_3_26_with_conn",
		&SQLiteDriver{
	...
	connect = func(t *testing.T, f string, username, password string) (file string, db *sql.DB, c *SQLiteConn, err error) {
		...
		db, err = sql.Open("sqlite3_3_26_with_conn", "file:"+file+params)
		if err != nil {
	...
	connectWithCrypt = func(t *testing.T, f string, username, password string, crypt string, salt string) (file string, db *sql.DB, c *SQLiteConn, err error) {
		...
		db, err = sql.Open("sqlite3_3_26_with_conn", "file:"+file+fmt.Sprintf("?_auth&_auth_user=%s&_auth_pass=%s&_auth_crypt=%s&_auth_salt=%s", username, password, crypt, salt))
		if err != nil {
			defer os.Remove(file)
			return file, nil, nil, err
		}
