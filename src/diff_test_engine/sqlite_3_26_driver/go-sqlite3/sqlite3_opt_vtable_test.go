...
func TestCreateModule(t *testing.T) {
	...
	intarray := []int{1, 2, 3}
	sql.Register("sqlite3_3_26_TestCreateModule", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
			return conn.CreateModule("test", testModule{t, intarray})
		},
	})
	db, err := sql.Open("sqlite3_3_26_TestCreateModule", tempFilename)
	if err != nil {
...
func TestVUpdate(t *testing.T) {
	...
	// register module
	sql.Register("sqlite3_3_26_TestVUpdate", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
	...
	// connect
	db, err := sql.Open("sqlite3_3_26_TestVUpdate", tempFilename)
	if err != nil {
		t.Fatalf("could not open db: %v", err)
	}
...
func TestCreateModuleEponymousOnly(t *testing.T) {
	...
	defer os.Remove(tempFilename)
	intarray := []int{1, 2, 3}
	sql.Register("sqlite3_3_26_TestCreateModuleEponymousOnly", &SQLiteDriver{
		ConnectHook: func(conn *SQLiteConn) error {
			return conn.CreateModule("test", testModuleEponymousOnly{t, intarray})
		},
	})
	db, err := sql.Open("sqlite3_3_26_TestCreateModuleEponymousOnly", tempFilename)
	if err != nil {
		t.Fatalf("could not open db: %v", err)
	}
