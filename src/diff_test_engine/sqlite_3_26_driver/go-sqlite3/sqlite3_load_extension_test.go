...
func TestExtensionsError(t *testing.T) {
	sql.Register("sqlite3_3_26_TestExtensionsError",
		&SQLiteDriver{
			Extensions: []string{
				"foobar",
			},
	...
	db, err := sql.Open("sqlite3_3_26_TestExtensionsError", ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
...
func TestLoadExtensionError(t *testing.T) {
	sql.Register("sqlite3_3_26_TestLoadExtensionError",
		&SQLiteDriver{
	...
	db, err := sql.Open("sqlite3_3_26_TestLoadExtensionError", ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
