...
static inline char *_sqlite3_3_26_mprintf(char *zFormat, char *arg) {
  return sqlite3_3_26_mprintf(zFormat, arg);
}
...
static int cXInit(sqlite3 *db, void *pAux, int argc, const char *const*argv, sqlite3_3_26_vtab **ppVTab, char **pzErr, int isCreate) {
	...
	if (!pvTab) {
		*pzErr = sqlite3_3_26_mprintf("%s", "Out of memory");
		return SQLITE_NOMEM;
	}
	memset(pvTab, 0, sizeof(goVTab));
	pvTab->vTab = vTab;

	*ppVTab = (sqlite3_3_26_vtab *)pvTab;
	*pzErr = 0;
	return SQLITE_OK;
}

static inline int cXCreate(sqlite3 *db, void *pAux, int argc, const char *const*argv, sqlite3_3_26_vtab **ppVTab, char **pzErr) {
	return cXInit(db, pAux, argc, argv, ppVTab, pzErr, 1);
}
static inline int cXConnect(sqlite3 *db, void *pAux, int argc, const char *const*argv, sqlite3_3_26_vtab **ppVTab, char **pzErr) {
	return cXInit(db, pAux, argc, argv, ppVTab, pzErr, 0);
}

char* goVBestIndex(void *pVTab, void *icp);

static inline int cXBestIndex(sqlite3_3_26_vtab *pVTab, sqlite3_3_26_index_info *info) {
...
static int cXRelease(sqlite3_3_26_vtab *pVTab, int isDestroy) {
...
static inline int cXDisconnect(sqlite3_3_26_vtab *pVTab) {
	return cXRelease(pVTab, 0);
}
static inline int cXDestroy(sqlite3_3_26_vtab *pVTab) {
...
static int cXOpen(sqlite3_3_26_vtab *pVTab, sqlite3_3_26_vtab_cursor **ppCursor) {
	...
	*ppCursor = (sqlite3_3_26_vtab_cursor *)pCursor;
	return SQLITE_OK;
}

static int setErrMsg(sqlite3_3_26_vtab_cursor *pCursor, char *pzErr) {
...
static int cXClose(sqlite3_3_26_vtab_cursor *pCursor) {
...
char* goVFilter(void *pCursor, int idxNum, char* idxName, int argc, sqlite3_3_26_value **argv);

static int cXFilter(sqlite3_3_26_vtab_cursor *pCursor, int idxNum, const char *idxStr, int argc, sqlite3_3_26_value **argv) {
...
static int cXNext(sqlite3_3_26_vtab_cursor *pCursor) {
...
static inline int cXEof(sqlite3_3_26_vtab_cursor *pCursor) {
	return goVEof(((goVTabCursor*)pCursor)->vTabCursor);
}

char* goVColumn(void *pCursor, void *cp, int col);

static int cXColumn(sqlite3_3_26_vtab_cursor *pCursor, sqlite3_3_26_context *ctx, int i) {
...
char* goVRowid(void *pCursor, sqlite3_3_26_int64 *pRowid);

static int cXRowid(sqlite3_3_26_vtab_cursor *pCursor, sqlite3_3_26_int64 *pRowid) {
...
char* goVUpdate(void *pVTab, int argc, sqlite3_3_26_value **argv, sqlite3_3_26_int64 *pRowid);

static int cXUpdate(sqlite3_3_26_vtab *pVTab, int argc, sqlite3_3_26_value **argv, sqlite3_3_26_int64 *pRowid) {
...
static int _sqlite3_3_26_create_module(sqlite3 *db, const char *zName, uintptr_t pClientData) {
  return sqlite3_3_26_create_module_v2(db, zName, &goModule, (void*) pClientData, goMDestroy);
}

static int _sqlite3_3_26_create_module_eponymous_only(sqlite3 *db, const char *zName, uintptr_t pClientData) {
  return sqlite3_3_26_create_module_v2(db, zName, &goModuleEponymousOnly, (void*) pClientData, goMDestroy);
}
*/
import "C"
...
func constraints(info *C.sqlite3_3_26_index_info) []InfoConstraint {
	slice := *(*[]C.struct_sqlite3_3_26_index_constraint)(unsafe.Pointer(&reflect.SliceHeader{
...
func orderBys(info *C.sqlite3_3_26_index_info) []InfoOrderBy {
	slice := *(*[]C.struct_sqlite3_3_26_index_orderby)(unsafe.Pointer(&reflect.SliceHeader{
...
func mPrintf(format, arg string) *C.char {
	...
	return C._sqlite3_3_26_mprintf(cf, ca)
}
...
func goVBestIndex(pVTab unsafe.Pointer, icp unsafe.Pointer) *C.char {
	vt := lookupHandle(pVTab).(*sqliteVTab)
	info := (*C.sqlite3_3_26_index_info)(icp)
	...
	slice := *(*[]C.struct_sqlite3_3_26_index_constraint_usage)(unsafe.Pointer(&reflect.SliceHeader{
	...
	info.idxStr = (*C.char)(C.sqlite3_3_26_malloc(C.int(len(res.IdxStr) + 1)))
	if info.idxStr == nil {
	...
	info.estimatedRows = C.sqlite3_3_26_int64(res.EstimatedRows)
...
//export goVFilter
func goVFilter(pCursor unsafe.Pointer, idxNum C.int, idxName *C.char, argc C.int, argv **C.sqlite3_3_26_value) *C.char {
	vtc := lookupHandle(pCursor).(*sqliteVTabCursor)
...
//export goVRowid
func goVRowid(pCursor unsafe.Pointer, pRowid *C.sqlite3_3_26_int64) *C.char {
	vtc := lookupHandle(pCursor).(*sqliteVTabCursor)
	rowid, err := vtc.vTabCursor.Rowid()
	if err != nil {
		return mPrintf("%s", err.Error())
	}
	*pRowid = C.sqlite3_3_26_int64(rowid)
	return nil
}

//export goVUpdate
func goVUpdate(pVTab unsafe.Pointer, argc C.int, argv **C.sqlite3_3_26_value, pRowid *C.sqlite3_3_26_int64) *C.char {
	vt := lookupHandle(pVTab).(*sqliteVTab)
	...
	if v, ok := vt.vTab.(VTabUpdater); ok {
		...
		case argc > 1 && vals[0] == nil:
			...
			if err == nil {
				*pRowid = C.sqlite3_3_26_int64(id)
			}
...
func (c *SQLiteConn) DeclareVTab(sql string) error {
	...
	rv := C.sqlite3_3_26_declare_vtab(c.db, zSQL)
	if rv != C.SQLITE_OK {
		return c.lastError()
	}
	return nil
...
func (c *SQLiteConn) CreateModule(moduleName string, module Module) error {
	...
	case EponymousOnlyModule:
		rv := C._sqlite3_3_26_create_module_eponymous_only(c.db, mname, C.uintptr_t(uintptr(newHandle(c, &udm))))
		if rv != C.SQLITE_OK {
			return c.lastError()
		}
		return nil
	case Module:
		rv := C._sqlite3_3_26_create_module(c.db, mname, C.uintptr_t(uintptr(newHandle(c, &udm))))
		if rv != C.SQLITE_OK {
			return c.lastError()
		}
		return nil
