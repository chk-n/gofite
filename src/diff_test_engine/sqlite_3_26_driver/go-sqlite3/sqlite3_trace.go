...
int traceCallbackTrampoline_new(unsigned int traceEventCode, void *ctx, void *p, void *x);
*/
import "C"
...
// for SQL Trace Hook = sqlite3_3_26_trace_v2():
...
func fillDBError(dbErr *Error, db *C.sqlite3) {
	...
	dbErr.Code = ErrNo(C.sqlite3_3_26_errcode(db))
	dbErr.ExtendedCode = ErrNoExtended(C.sqlite3_3_26_extended_errcode(db))
	dbErr.err = C.GoString(C.sqlite3_3_26_errmsg(db))
}
...
func fillExpandedSQL(info *TraceInfo, db *C.sqlite3, pStmt unsafe.Pointer) {
	...
	expSQLiteCStr := C.sqlite3_3_26_expanded_sql((*C.sqlite3_3_26_stmt)(pStmt))
	defer C.sqlite3_3_26_free(unsafe.Pointer(expSQLiteCStr))
...
//export traceCallbackTrampoline_new
func traceCallbackTrampoline_new(
	...
	info.AutoCommit = (int(C.sqlite3_3_26_get_autocommit(contextDB)) != 0)
	...
	// registering this callback trampoline_new with SQLite --- for cleanup.
...
func (c *SQLiteConn) SetTrace(requested *TraceConfig) error {
	...
	// The callback trampoline_new function does cleanup on Close event,
...
func (c *SQLiteConn) setSQLiteTrace(sqliteEventMask uint) error {
	rv := C.sqlite3_3_26_trace_v2(c.db,
		C.uint(sqliteEventMask),
		(*[0]byte)(unsafe.Pointer(C.traceCallbackTrampoline_new)),
